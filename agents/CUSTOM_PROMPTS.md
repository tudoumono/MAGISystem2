...
- SOLOMONプロンプトの `{sage_responses}` プレースホルダーは**省略可能**です（自動的に末尾に追加されます）
...
回答の文字数制限を環境変数でカスタマイズできます：

```bash
# 3賢者のreasoningフィールドの最大文字数（デフォルト: 1000文字）
SAGE_REASONING_MAX_LENGTH=1000

# SOLOMONのreasoningフィールドの最大文字数（デフォルト: 1500文字）
SOLOMON_REASONING_MAX_LENGTH=1500
```

**推奨値：**
- **簡潔な回答が必要な場合**: 500-800文字
- **標準（推奨）**: 1000-1500文字
- **詳細な分析が必要な場合**: 2000-3000文字

**注意：**
- 文字数を増やすと、より詳細な分析が得られますが、LLMのトークン消費も増加します
- フロントエンドでの表示も考慮して設定してください
...
```json
{
  ...
  "reasoning": "判断理由（SAGE_REASONING_MAX_LENGTH文字以内、デフォルト: 1000）",
  "confidence": 0.0-1.0
}
```
...
```json
{
  ...
  "reasoning": "統合判断の理由（SOLOMON_REASONING_MAX_LENGTH文字以内、デフォルト: 1500）",
  "confidence": 0.0-1.0,
  "sage_scores": {
    "caspar": 0-100,
    "balthasar": 0-100,
...
### SOLOMONプロンプトの{sage_responses}について

- `{sage_responses}` プレースホルダーは**省略可能**です
- プレースホルダーが含まれていない場合、自動的に末尾に以下の形式で追加されます：
  ```
  【入力】
  3賢者の判断結果：
  {sage_responses}
  ```
- 独自の形式で3賢者の結果を受け取りたい場合は、明示的にプレースホルダーを含めてください

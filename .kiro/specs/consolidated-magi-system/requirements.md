...
## 設計の参考元
この要件定義は以下の記事のアーキテクチャパターンを前提としています：
- **参考記事**: [Amplify HostingでBedrock AgentCoreを使う](https://qiita.com/moritalous/items/ea695f8a328585e1313b)
- **前提アーキテクチャ**: フロントエンド/バックエンド分離、AgentCore Runtime標準仕様準拠
- **技術的制約**: Amplify Hostingのストリーミング制限を考慮した要件設計
...
### 3.6 Strands Agents完全統合要件（必須）
- **完全実装**: 全エージェント（CASPAR、BALTHASAR、MELCHIOR、SOLOMON）でStrands Agents SDKを必ず使用
- **禁止パターン**: 
  - ❌ 独自エージェントフレームワークの実装禁止
  - ❌ boto3直接呼び出しによるBedrock API使用禁止
  - ❌ LangChainやその他フレームワークへの置き換え禁止
  - ❌ キーワードベースのモックロジック実装禁止
- **必須パターン**:
  - ✅ `from strands import Agent`の使用
  - ✅ `from strands.models import BedrockModel`の使用
  - ✅ Strands Agentsの`@tool`デコレータによるツール定義
  - ✅ 非同期処理（`async/await`）の活用
- **検証要件**: 
  - 実装時にStrands Agents使用を必ず確認
  - コードレビュー時の禁止パターンチェック
  - CI/CDパイプラインでの自動検証
- **ステアリング連携**: AI提案時のチェックリスト適用（tech.md参照）
- **公式ドキュメント参照**: https://strandsagents.com/latest/
